...
public final class BanCommand {
    ...
    public void onDefaultExecution(BukkitSource source) {
        ...
        TextHelper.sendPrefixedMessage(player, "&eUsage: /ban (target) [-silent/-s] [-ip] [duration] [reason]");
    }
    ...
    public void ban(BukkitSource source, OfflinePlayer target,
        ...
        if (ip) {
            ...
            if (onlineTarget == null) {
                TextHelper.sendPrefixedMessage(source.asPlayer(), "&cTarget must be online to IP ban.");
                return;
            }
        ...
        Punishment punishment = new Punishment(source.asPlayer().getUniqueId(), target.getUniqueId(),
                Punishment.PunishmentType.BAN, finalReason, ipAddress, System.currentTimeMillis(), banDuration, ip);
